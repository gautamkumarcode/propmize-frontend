      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-xl shadow p-6 border-t-4 border-blue-500">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-xl text-gray-800">Basic Details</h3>
          </div>
        ...
        <div className="bg-white rounded-xl shadow p-6 border-t-4 border-green-500">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-xl text-gray-800">Property Features</h3>
          </div>
          {values.features ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <span className="text-muted-foreground text-base">No features specified</span>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow p-6 border-t-4 border-red-500">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {Array.isArray(values.images) && values.images.length > 0 ? (
              ...
              <span className="text-muted-foreground text-base">No images uploaded</span>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            {Array.isArray(values.videos) && values.videos.length > 0 ? (
              ...
              <span className="text-muted-foreground text-base">No videos uploaded</span>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow p-6 border-t-4 border-teal-500">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-xl text-gray-800">Pricing & Legal</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <DetailItem
              label="Base Price"
              value={`${values.pricing?.basePrice?.value || ""} ${values.pricing?.basePrice?.unit || ""}`}
            />
            <DetailItem
              label="Maintenance Charges"
              value={`${values.pricing?.maintenanceCharges?.value || ""} ${values.pricing?.maintenanceCharges?.unit || ""}`}
            />
            <DetailItem
              label="Security Deposit"
              value={`${values.pricing?.securityDeposit?.value || ""} ${values.pricing?.securityDeposit?.unit || ""}`}
            />
            <DetailItem
        ...
        <div className="bg-white rounded-xl shadow p-6 border-t-4 border-orange-500">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-xl text-gray-800">Contact & Notes</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <h4 className="font-semibold text-lg mt-6 mb-3 text-gray-800">Nearby Places</h4>
          {values.nearbyPlaces ? (
            <div className="space-y-2">
              {Object.entries(values.nearbyPlaces).map(
                ([key, places]: [string, any]) => (
                  <div key={key}>
                    <span className="font-semibold capitalize text-md text-gray-700">{key}:</span>
