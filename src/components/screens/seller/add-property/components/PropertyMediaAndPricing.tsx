...
type PricingValueFieldKey = `pricing.${PricingFieldNames}.value`;
type PricingUnitFieldKey = `pricing.${PricingFieldNames}.unit`;
				...
				<FormField
					...
					render={() => (
						<FormItem>
							...
							<div className="text-xs text-muted-foreground mt-2">
								You can upload up to 5 videos. Supported formats: MP4, WEBM,
								MOV.
							</div>
							<FormMessage />
						</FormItem>
			...
			{priceFields.map((input) => (
				<div key={input.name} className="flex flex-col gap-1">
					...
					<div className="flex w-full items-center bg-background border rounded-lg overflow-hidden">
						<FormField
							control={form.control}
							name={`pricing.${input.name}.value` as PricingValueFieldKey}
							render={({ field }) => (
						...
						<FormField
							control={form.control}
							name={`pricing.${input.name}.unit` as PricingUnitFieldKey}
